...
def CreateLog(FolderName):
    ...
    # FIX 1: invalid characters removed from timestamp
    timestamp = time.strftime("%Y_%m_%d_%H_%M_%S")
    
    FileName = os.path.join(FolderName, "Marvellous_%s.log" % timestamp)
    print(FileName)
    
    # FIX 2: file closed properly
...
def main():
    ...
    if(len(sys.argv) == 2):
        ...
        print("Platform surevllance system started sucessfully")
        print("Directory created with naem : ", sys.argv[2])
        print("Time interval : ", sys.argv[1])
